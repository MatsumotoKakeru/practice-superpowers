...
UIの画面を含む機能を設計する場合、設計書には画面ごとに以下のセクションを含める。**コード・実装方法・APIエンドポイント・テスト方針は書かない。**
...
## 項目一覧
- 項目名（必須/任意・表示形式・文字制限）：項目の意味。バリデーションルール。

## イベント処理
- アクション名：
  - チェック：バリデーション・条件チェックの内容
  - 正常：成功時に何が起きるか
  - エラー：失敗時に何が起きるか
```
...
- 言語（必須・テキスト入力・文字制限なし）：コードのプログラミング言語。空欄不可。
- スタイル（任意・テキスト入力・文字制限なし）：コードの表示スタイル。
- 行番号表示（任意・チェックボックス）：行番号を表示するかどうか。

**イベント処理の例：**
- 保存ボタン押下：
  - チェック：必須項目が空欄の場合はエラーメッセージを表示して送信しない
  - 正常：入力内容を保存してスニペット一覧画面に戻る
  - エラー：サーバーエラーの場合はエラーメッセージを画面上部に表示する
- 一覧に戻るリンク押下：入力内容を破棄してスニペット一覧画面に戻る

**禁止事項（設計書に書いてはいけないもの）:**
- TypeScript / Python などのコードブロック
- ファイルパスや関数名
- APIエンドポイントの仕様
- テスト方針
